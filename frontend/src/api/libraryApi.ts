...
/**
 * Returns HTTP headers containing a bearer Authorization header when an access token is available in the browser.
 *
 * Checks for an `accessToken` in `localStorage` when running in a browser environment and, if present, returns
 * an object with an `Authorization` header set to `Bearer <token>`. If no token is available or not running in a
 * browser, returns an empty object.
 *
 * @returns An object with an `Authorization` header containing the bearer token if found, or an empty object otherwise.
 */
