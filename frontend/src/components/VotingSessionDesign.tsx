        ...
        <div className="max-w-screen-2xl mx-auto px-4 py-8">
            ...
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-2">
                    {[1, 2, 3, 4, 5].map((id) => (
                        <img key={id} src={`/avatars/${id}.jpg`} alt="User" className="w-10 h-10 rounded-full" />
                    ))}
            ...
            {/* Restaurant Card */}
            <img src="#" alt="Restaurant" className="w-full rounded-xl mb-4" />
            <h3 className="text-xl font-semibold">The Spice Merchant</h3>
            <p className="text-sm text-gray-500 mb-2">4.5 ⭐⭐⭐⭐ · $$ · Downtown</p>
            <p className="text-gray-600 mb-6">Authentic Indian cuisine with a modern twist. Enjoy a variety of flavorful dishes in a cozy atmosphere.</p>

            {/* Menu Highlights */}
            <h4 className="font-medium mb-2">Menu Highlights</h4>
            <div className="flex gap-4 mb-6">
                {["Chicken Tikka Masala", "Vegetable Biryani", "Garlic Naan"].map((item, index) => (
                    <div key={item} className="w-1/3 text-center">
                        <img src={`/dishes/${index + 1}.jpg`} alt={item} className="rounded-lg mb-2" />
            ...
            {/* Review Dropdown */}
            <div className="mb-6">
                <label htmlFor="review" className="block text-sm font-medium mb-1">Select a Review</label>
